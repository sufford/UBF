...
#define MOD_TAPER_MIRROR 1 //no idea why deepseek asked me to include it lol
...
typedef enum ModifierType {
	...
	eModifierType_Noise 			= 54,
	eModifierType_CapHoles			= 55,
	eModifierType_Spherify			= 56,
	eModifierType_Push				= 57,
	eModifierType_Twist				= 58,
	eModifierType_DeleteMesh		= 59,
	eModifierType_Taper				= 60,
...
/* Array shape types */
#define MOD_ARR_SHAPE_LINE      0
#define MOD_ARR_SHAPE_CIRCLE    1
#define MOD_ARR_SHAPE_CURVE     2
#define MOD_ARR_SHAPE_TRANSFORM 3

/* Array transform reference types */
#define MOD_ARR_TRANSFORM_LINEAR 0
#define MOD_ARR_TRANSFORM_OBJECT 1

/* Array randomizer scale mode */
#define MOD_ARR_RANDOM_SCALE_UNIFORM 0
#define MOD_ARR_RANDOM_SCALE_AXES    1

/* Array circle fill types */
#define MOD_ARR_CIRCLE_FULL 0
#define MOD_ARR_CIRCLE_ARC  1
...
typedef struct ArrayModifierData {
	...
	float circle_radius;
	float circle_angle;
	float transform_rot[3];
	float transform_scale[3];
	int circle_axis;
	int circle_fill_type;
	int transform_ref;  /* 0=LINEAR, 1=OBJECT */
	int pad2;
	
	/* Randomizer section */
	float random_offset[3];
	float random_rot[3];
	float random_scale[3];
	float random_scale_uniform;
	int random_flip;
	int random_exclude_first;
	int random_exclude_last;
	int random_seed;
	int pad3;
	int pad4;
	int pad5; //imagine youre a solo coding this and getting bunch of errors that exceeds over 100 and u can figure out what causes it until you realise its not 8 bytes
} ArrayModifierData;
...
/* MirrorModifierData->flag2 */
#define MOD_MIR_BISECT_AXIS_X      (1 << 0)
#define MOD_MIR_BISECT_AXIS_Y      (1 << 1)
#define MOD_MIR_BISECT_AXIS_Z      (1 << 2)
#define MOD_MIR_BISECT_FLIP_X      (1 << 3)
#define MOD_MIR_BISECT_FLIP_Y      (1 << 4)
#define MOD_MIR_BISECT_FLIP_Z      (1 << 5)
...
typedef struct CapHolesModifierData {
    ModifierData modifier;
    ...
    int pad;
    int pad2; 
} CapHolesModifierData;

typedef struct SpherifyModifierData {
    ModifierData modifier;   /* 24 bytes */
    float percent;            /* 4 bytes (total: 28) */
    float radius;             /* 4 bytes (total: 32) */
    int pad;                  /* 4 bytes (total: 36) */
    int pad2;                 /* 4 bytes (total: 40) - Multiple of 8! */
} SpherifyModifierData;

typedef struct PushModifierData {
    ModifierData modifier;
    float distance;
    char _pad[4]; /* Required padding to align memory blocks in Blender 2.79 */
} PushModifierData;

typedef struct TwistModifierData {
    ModifierData modifier; /* First entry rule */
    
    float angle;           /* 4 bytes */
    int axis;              /* 4 bytes */
    
    /* Perfect alignment! No manual padding variables are needed here at all */
} TwistModifierData;

typedef struct TaperModifierData {
    ModifierData modifier;
    
    float amount;     /* 4 bytes */
    float curve;      /* 4 bytes */
    int axis;         /* 4 bytes - Primary Axis (0=X, 1=Y, 2=Z) */
    int effect_axis;  /* 4 bytes - Effect Axis  (0=X, 1=Y, 2=XY) */
    int flag;         /* 4 bytes - Bitmask options */
    int _pad;         /* 4 bytes - Restores 8-byte structure boundary matching */
} TaperModifierData;

/* Bitmask layout token definition */
#define MOD_TAPER_SYMMETRY (1 << 0)

/* Squeeze Modifier flags */
#define MOD_SQUEEZE_VOLUME (1 << 0)

typedef struct SqueezeModifierData {
    ModifierData modifier;
...
typedef enum {
    MOD_WAVEMAX_MODE_WAVE   = 0,
    MOD_WAVEMAX_MODE_RIPPLE = 1,
    MOD_WAVEMAX_MODE_MIXED  = 2
} WaveMaxMode;

typedef struct WaveMaxModifierData {
    ModifierData modifier;
    
    float amplitude[2];   /* Keeps your original array intact */
    float wave_length;
    float phase;
    float decay;
    int axis;
    
    int mode;            /* Replaced pad: Stores WaveMaxMode enum */
    float blend;         /* Replaced pad2: Controls hybrid mix amount (0.0 to 1.0) */
} WaveMaxModifierData;

typedef enum {
    MOD_MELT_SOL_ICE     = 0,
    MOD_MELT_SOL_GLASS   = 1,
    MOD_MELT_SOL_JELLY   = 2,
    MOD_MELT_SOL_PLASTIC = 3,
    MOD_MELT_SOL_CUSTOM  = 4
} MeltSolidityPreset;

typedef struct MeltModifierData {
    ModifierData modifier;
    
    struct Object *gizmo;   /* Empty object reference */
    
    float amount;           /* Melt amount */
    float spread;           /* Base spreading */
    float solidity_val;     /* Custom solidity exponent */
    int solidity_type;      /* 0=ICE, 1=GLASS, 2=JELLY, 3=PLASTIC, 4=CUSTOM */
...
typedef struct DeleteMeshModifierData {
    ModifierData modifier;
