...
static struct WMInitStruct {
	...
	bool window_focus;
	bool native_pixels;
} wm_init_state = {0, 0, 0, 0, GHOST_kWindowStateMaximized, 0, true, true};
...
static uiBlock *block_create_confirm_quit(struct bContext *C, struct ARegion *ar, void *UNUSED(arg1))
{
	...
	but = uiDefIconTextBut(
	        block, UI_BTYPE_BUT, 0, ICON_FILE_TICK, IFACE_("Save"), 0, 0, 50, UI_UNIT_Y,
	        NULL, 0, 0, 0, 0, TIP_("Save and quit"));
	UI_but_func_set(but, wm_block_confirm_quit_save, block, NULL);

	col = uiLayoutColumn(split, 1);
	but = uiDefIconTextBut(
	        block, UI_BTYPE_BUT, 0, ICON_CANCEL, IFACE_("Don't Save"), 0, 0, 50, UI_UNIT_Y,
...
void wm_window_title(wmWindowManager *wm, wmWindow *win)
{
	if (win->screen && win->screen->temp) {
		...
		if (G.save_over && BKE_main_blendfile_path_from_global()[0]) {
			char str[sizeof(((Main *)NULL)->name) + 32];
			BLI_snprintf(str, sizeof(str), "Untitled Blender Fork%s [%s%s]", wm->file_saved ? "" : "*",
			             BKE_main_blendfile_path_from_global(),
			             G_MAIN->recovered ? " (Recovered)" : "");
			GHOST_SetTitle(win->ghostwin, str);
		}
		else
			GHOST_SetTitle(win->ghostwin, "Untitled Blender Fork");
...
void wm_window_ghostwindows_ensure(wmWindowManager *wm)
{
	...
	for (win = wm->windows.first; win; win = win->next) {
		if (win->ghostwin == NULL) {
			...
			/* should force it to be maximised im fuckin tired of constantly maximising every time ts runs */
			win->windowstate = GHOST_kWindowStateMaximized;

			wm_window_ghostwindow_add(wm, "Untitled Blender Fork", win);
		}
...
wmWindow *WM_window_open_temp(bContext *C, int x, int y, int sizex, int sizey, int type)
{
	...
	if (sa->spacetype == SPACE_IMAGE)
		title = IFACE_("Untitled Blender Fork Render");
	else if (ELEM(sa->spacetype, SPACE_OUTLINER, SPACE_USERPREF))
		title = IFACE_("Untitled Blender Fork User Preferences");
	else if (sa->spacetype == SPACE_FILE)
		title = IFACE_("Untitled Blender Fork File View");
	else
		title = "Untitled Blender Fork";
