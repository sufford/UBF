...
static void backdrawview3d(Scene *scene, wmWindow *win, ARegion *ar, View3D *v3d, Object *obact, Object *obedit)
{
	...
	if (obact && view3d_base_visible(scene, v3d, BKE_scene_base_find(scene, obact))) {
...
static void view3d_draw_depth_loop(Main *bmain, Scene *scene, ARegion *ar, View3D *v3d)
{
	...
	for (base = scene->base.first; base; base = base->next) {
		if (view3d_base_visible(scene, v3d, base)) {
			/* Ensure object matrix is up to date for dynamic layers */
			if (base->object->layer_links.first) {
				BKE_object_where_is_calc(scene, base->object);
			}
        ...
        const bool do_bgpic, const bool draw_offscreen, GPUFX *fx)
{
	...
	if (draw_offscreen) {
		for (base = scene->base.first; base; base = base->next) {
			if (view3d_base_visible(scene, v3d, base)) {
				/* Force matrix update for dynamic layers */
				if (base->object->layer_links.first) {
					BKE_object_where_is_calc(scene, base->object);
				}
