	...
	CTX_DATA_BEGIN(C, Base *, base, visible_bases)
	{
		...
		if (layernum < 20) {
			...
			case OB_SEL_LAYERMATCH_EXACT:
				ok = ((base->lay & ((1 << 20) - 1)) == (1 << layernum));
				break;
			case OB_SEL_LAYERMATCH_SHARED:
				ok = (base->lay & (1 << layernum)) != 0;
				break;
			}
		}
		else {
			/* Dynamic layers - check layer_links */
			SceneLayer *target_sl = BKE_scene_layer_find_index(scene, layernum);
			if (target_sl) {
				ObjectLayerLink *link;
				for (link = base->object->layer_links.first; link; link = link->next) {
					if (link->layer == target_sl) {
						ok = true;
						break;
					}
				}
...
void OBJECT_OT_select_by_layer(wmOperatorType *ot)
{
	...
	RNA_def_boolean(ot->srna, "extend", false, "Extend", "Extend selection instead of deselecting everything first");
	RNA_def_int(ot->srna, "layers", 1, 1, INT_MAX, "Layer", "", 1, INT_MAX);
}
