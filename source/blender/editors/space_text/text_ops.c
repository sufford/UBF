...
extern const char* KXScript_RunString(const char* code);
static int text_run_kxscript_exec(bContext *C, wmOperator *op);
...
static int text_run_script_exec(bContext *C, wmOperator *op)
{
	...
	/* Check if it's a KXScript file */
	if (text) {
		bool is_kx = false;
		
		/* Check internal name (text->id.name) */
		if (strstr(text->id.name + 2, ".kx")) {
			is_kx = true;
		}
		
		/* Check file name (text->name) */
		if (text->name && strstr(text->name, ".kx")) {
			is_kx = true;
		}
		
		if (is_kx) {
			return text_run_kxscript_exec(C, op);
		}
	}
...
static int text_run_kxscript_exec(bContext *C, wmOperator *op)
{
    ...
    /* Run KXScript and get output */
    const char* output = KXScript_RunString(buf);
    
    /* Report each line separately */
    if (output && output[0]) {
        char *output_copy = BLI_strdup(output);
        char *line = strtok(output_copy, "\n");
        
        while (line != NULL) {
            if (line[0] != '\0') {
                BKE_report(op->reports, RPT_INFO, line);
            }
            line = strtok(NULL, "\n");
        }
        
        MEM_freeN(output_copy);
    }
    else {
        BKE_report(op->reports, RPT_INFO, "KXScript executed");
    }
...
void TEXT_OT_run_kxscript(wmOperatorType *ot)
{
    /* identifiers */
    ot->name = "Run KXScript";
    ot->idname = "TEXT_OT_run_kxscript";
    ot->description = "Run active KXScript";
    
    /* api callbacks */
    ot->exec = text_run_kxscript_exec;
    ot->poll = text_run_script_poll;
