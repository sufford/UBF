...
class DATA_PT_modifiers(ModifierButtonsPanel, Panel):
    ...
    def ARRAY(self, layout, ob, md):
        # Shape section
        layout.prop(md, "shape_type")
        
        if md.shape_type == 'CIRCLE':
            # --- CLEAN NATIVE USER INTERFACE PANELS ---
            col = layout.column(align=True)
            
            # BRING BACK THE AXIS CONTROL SELECTOR
            col.row().prop(md, "circle_axis", expand=True) # expand=True turns it into punchy X | Y | Z button tabs!
            
            col.prop(md, "circle_radius", text="Radius")
            
            layout.separator()
            layout.prop(md, "count", text="Count")
            
            # Circle Segment section
            layout.separator()
            layout.prop(md, "circle_fill_type", text="Segment")
            if md.circle_fill_type == 'ARC':
                layout.prop(md, "circle_sweep_angle", text="Sweep Angle")
            
            
        elif md.shape_type == 'CURVE':
            # --- FIXED: NATIVE CURVE PANEL VIEW ---
            col = layout.column()
            col.prop(md, "curve", text="Curve Target") # Draws the object pointer selector field
            col.prop(md, "count", text="Count")       # Draws our explicit duplicate count slider
            
            layout.separator()
            
            # Keep merge active for curves since closed loops can weld edges too!
            row = layout.row()
            row.prop(md, "use_merge_vertices", text="Merge")
            if md.use_merge_vertices:
                row.prop(md, "merge_threshold", text="Distance")
        elif md.shape_type == 'TRANSFORM':
            layout.prop(md, "count")
            layout.prop(md, "transform_ref")
            if md.transform_ref == 'LINEAR':
                layout.prop(md, "offset")
                layout.prop(md, "transform_rot")
                layout.prop(md, "transform_scale")
            elif md.transform_ref == 'OBJECT':
                layout.prop(md, "offset_object")
        else:
            # LINE mode (original layout)
            ...
            # New: Rotation and Scale for LINE mode
            layout.separator()
            col = layout.column(align=True)
            col.prop(md, "transform_rot", text="Rotation")
            col.prop(md, "transform_scale", text="Scale")
        ...
        layout.separator()
        box = layout.box()
        box.label(text="Randomizer:")
        box.prop(md, "random_offset")
        box.prop(md, "random_rot")
        box.prop(md, "random_scale")
        box.prop(md, "random_flip")
        row = box.row()
        row.prop(md, "random_exclude_first")
        row.prop(md, "random_exclude_last")
        box.prop(md, "random_seed")
    ...
    def MIRROR(self, layout, ob, md):
        ...
        col.prop(md, "use_bisect_x", text="X")
        col.prop(md, "use_bisect_y", text="Y")
        col.prop(md, "use_bisect_z", text="Z")
        
        col.separator()
        col.label(text="Flip:")
        col.prop(md, "use_bisect_flip_x", text="X")
        col.prop(md, "use_bisect_flip_y", text="Y")
        col.prop(md, "use_bisect_flip_z", text="Z")
        ...
        col.prop(md, "bisect_threshold", text="Bisect Distance")
    ...
    def NOISE(self, layout, ob, md):
        ...
        box.prop(md, "fractal")
        if md.fractal:
            box.prop(md, "roughness")
            box.prop(md, "iterations")

    def CAP_HOLES(self, layout, ob, md):
        layout.prop(md, "smooth")
        layout.prop(md, "triangulate")
    
    def SPHERIFY(self, layout, ob, md):
        layout.prop(md, "percent")
        layout.prop(md, "radius")
    
    def PUSH(self, layout, ob, md):
        layout.prop(md, "distance")
    
    def TWIST(self, layout, ob, md):
        layout.prop(md, "angle")
        layout.prop(md, "center")
        layout.prop(md, "axis")
    
    def DELETE_MESH(self, layout, ob, md):
        layout.label(text="Deletes all geometry")
    
    def TAPER(self, layout, ob, md):
        layout.prop(md, "amount")
        layout.prop(md, "curve")
        layout.prop(md, "axis", expand=True) # expand=True turns X/Y/Z into a row of clickable radio buttons
        layout.prop(md, "effect_axis")       # Draws your new X, Y, XY dropdown selection box
        layout.prop(md, "taper_symmetry")    # Draws your newly resolved Symmetry checkbox toggle
    
    def SQUEEZE(self, layout, ob, md):
        # Radial Squeeze Group
        box = layout.box()
        box.label(text="Radial Squeeze:")
        box.prop(md, "amount", text="Amount")
        box.prop(md, "radial_curve", text="Curve")
    
        # Axial Bulge Group
        box = layout.box()
        box.label(text="Axial Bulge:")
        box.prop(md, "axial_amount", text="Amount")
        box.prop(md, "axial_curve", text="Curve")
    
        # Options
        layout.prop(md, "axis", expand=True)
        layout.prop(md, "use_volume_preserve")
    
    def WAVE_MAX(self, layout, ob, md):
        # 1. Standard parameters grouped together
        layout.prop(md, "amplitude")
        layout.prop(md, "wave_length")
        layout.prop(md, "phase")
        layout.prop(md, "decay")
        
        layout.separator()
        
        # 2. Main alignment axis buttons displayed side-by-side
        layout.label(text="Travel Axis:")
        layout.prop(md, "axis", expand=True)
        
        layout.separator()
        
        # 3. Mode Selection buttons displayed side-by-side
        layout.label(text="Wave Profile Mode:")
        layout.prop(md, "mode", expand=True)
        
        # 4. Contextual UI control: Only show/enable Blend if Mixed Mode (index 2) is active
        col = layout.column()
        col.prop(md, "blend", text="Blend Amount")
        col.active = (md.mode == 'MIXED')
    
    def MELT(self, layout, ob, md):
        # 1. Main adjustment factors
        layout.prop(md, "amount", slider=True)
        layout.prop(md, "spread", slider=True)
        
        layout.separator()
        
        # 2. Gravity alignment options displayed side-by-side
        layout.label(text="Gravity Direction:")
        layout.prop(md, "axis", expand=True)
        
        layout.separator()
        
        # 3. Solidity Material selection block
        layout.label(text="Material Solidity:")
        layout.prop(md, "solidity_type", text="")
        
        # 4. Contextual UI control: Only enable custom slider if 'CUSTOM' preset is selected
        col = layout.column()
        col.prop(md, "solidity_val", text="Solidity Exponent")
        col.active = (md.solidity_type == 'CUSTOM')
    ...
    def TRIANGULATE(self, layout, ob, md):
        layout.prop(md, "quad_method", text="Quad Method")
        layout.prop(md, "ngon_method", text="Ngon Method")
        layout.prop(md, "min_vertices", text="Minimum Vertices")
