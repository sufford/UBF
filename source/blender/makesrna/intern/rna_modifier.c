...
const EnumPropertyItem rna_enum_object_modifier_type_items[] = {
	...
	{eModifierType_Noise, "NOISE", ICON_MOD_WAVE, "Noise", ""},  
	{eModifierType_CapHoles, "CAP_HOLES", ICON_MOD_MASK, "Cap Holes", ""},
	{eModifierType_Spherify, "SPHERIFY", ICON_MOD_CAST, "Spherify", ""},
	{eModifierType_Twist, "TWIST", ICON_MOD_SIMPLEDEFORM, "Twist", ""},
	{eModifierType_Push, "PUSH", ICON_MOD_DISPLACE, "Push", ""},
	{eModifierType_DeleteMesh, "DELETE_MESH", ICON_MOD_MASK, "Delete Mesh", ""},
	{eModifierType_Taper, "TAPER", ICON_MOD_SIMPLEDEFORM, "Taper", ""},
	{eModifierType_Squeeze, "SQUEEZE", ICON_MOD_SIMPLEDEFORM, "Squeeze", "Squeeze modifier"},
	{eModifierType_Melt, "MELT", ICON_MOD_SIMPLEDEFORM, "Melt", ""},
	{eModifierType_Warp, "WARP", ICON_MOD_WARP, "Warp", ""},
	{eModifierType_Wave, "WAVE", ICON_MOD_WAVE, "Wave", ""},
	{eModifierType_WaveMax, "WAVE_MAX", ICON_MOD_WAVE, "Wave Max", ""},
	{0, "", 0, N_("Simulate"), ""},
...
static StructRNA *rna_Modifier_refine(struct PointerRNA *ptr)
{
	...
	switch ((ModifierType)md->type) {
		...
		case eModifierType_CapHoles:
			return &RNA_CapHolesModifier;
		case eModifierType_Spherify:
			return &RNA_SpherifyModifier;
		case eModifierType_Push:
			return &RNA_PushModifier;
		case eModifierType_Twist:
			return &RNA_TwistModifier;
		case eModifierType_DeleteMesh:
			return &RNA_DeleteMeshModifier;
		case eModifierType_Taper:
			return &RNA_TaperModifier;
		case eModifierType_Squeeze:
			return &RNA_SqueezeModifier;
		case eModifierType_WaveMax:
...
static void rna_def_modifier_mirror(BlenderRNA *brna)
{
	...
	//vibecoded bisect here
	prop = RNA_def_property(srna, "use_bisect_x", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag2", MOD_MIR_BISECT_AXIS_X);
	RNA_def_property_ui_text(prop, "Bisect X", "Cut along X mirror plane");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	prop = RNA_def_property(srna, "use_bisect_y", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag2", MOD_MIR_BISECT_AXIS_Y);
	RNA_def_property_ui_text(prop, "Bisect Y", "Cut along Y mirror plane");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	prop = RNA_def_property(srna, "use_bisect_z", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag2", MOD_MIR_BISECT_AXIS_Z);
	RNA_def_property_ui_text(prop, "Bisect Z", "Cut along Z mirror plane");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	prop = RNA_def_property(srna, "use_bisect_flip_x", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag2", MOD_MIR_BISECT_FLIP_X);
	RNA_def_property_ui_text(prop, "Flip X", "Flip bisect side on X");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	prop = RNA_def_property(srna, "use_bisect_flip_y", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag2", MOD_MIR_BISECT_FLIP_Y);
	RNA_def_property_ui_text(prop, "Flip Y", "Flip bisect side on Y");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	prop = RNA_def_property(srna, "use_bisect_flip_z", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag2", MOD_MIR_BISECT_FLIP_Z);
	RNA_def_property_ui_text(prop, "Flip Z", "Flip bisect side on Z");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");
	...
	RNA_def_property_ui_text(prop, "Bisect Distance", "Distance from mirror plane for cutting");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");
...
static void rna_def_modifier_array(BlenderRNA *brna)
{
	...
	/* Move ALL static enum arrays up here at the top of the scope block */
	static const EnumPropertyItem rna_enum_array_shape_items[] = {
		{MOD_ARR_SHAPE_LINE, "LINE", 0, "Line", "Arrange copies along a line"},
		{MOD_ARR_SHAPE_CIRCLE, "CIRCLE", 0, "Circle", "Arrange copies in a circle"},
		{MOD_ARR_SHAPE_CURVE, "CURVE", 0, "Curve", "Arrange copies natively along a curve"},
		{MOD_ARR_SHAPE_TRANSFORM, "TRANSFORM", 0, "Transform", "Sequential transformation"},
		{0, NULL, 0, NULL, NULL}
	};
	
	/* FIXED: Moved to the top to resolve C2065 Undeclared Identifier */
	static const EnumPropertyItem rna_enum_circle_axis_items[] = {
		{0, "X", 0, "X", "Rotate array around the X axis"},
		{1, "Y", 0, "Y", "Rotate array around the Y axis"},
		{2, "Z", 0, "Z", "Rotate array around the Z axis"},
		{0, NULL, 0, NULL, NULL}
	};
	...
	static const EnumPropertyItem rna_enum_transform_ref_items[] = {
		{MOD_ARR_TRANSFORM_LINEAR, "LINEAR", 0, "Linear", "Use offset/rotation/scale values"},
		{MOD_ARR_TRANSFORM_OBJECT, "OBJECT", 0, "Object", "Use object transform as progression"},
		{0, NULL, 0, NULL, NULL}
	};
	
	static const EnumPropertyItem rna_enum_circle_fill_items[] = {
		{MOD_ARR_CIRCLE_FULL, "FULL", 0, "Full", "Complete 360 degree circle"},
		{MOD_ARR_CIRCLE_ARC, "ARC", 0, "Arc", "Partial arc with sweep angle"},
		{0, NULL, 0, NULL, NULL}
	};
	
	

	/* Executive initialization logic code starts here */
	...
	RNA_def_property_enum_items(prop, rna_enum_array_shape_items);
	RNA_def_property_ui_text(prop, "Shape", "How copies are arranged");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	/* === ADDED: Circle Axis property configuration definitions === */
	prop = RNA_def_property(srna, "circle_axis", PROP_ENUM, PROP_NONE);
	RNA_def_property_enum_sdna(prop, NULL, "circle_axis");
	RNA_def_property_enum_items(prop, rna_enum_circle_axis_items); /* Works cleanly now! */
	RNA_def_property_ui_text(prop, "Circle Axis", "Axis of rotation layout");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	prop = RNA_def_property(srna, "circle_radius", PROP_FLOAT, PROP_DISTANCE);
	RNA_def_property_float_sdna(prop, NULL, "circle_radius");
	RNA_def_property_range(prop, 0.0f, FLT_MAX);
	RNA_def_property_ui_text(prop, "Radius", "Circle radius");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	prop = RNA_def_property(srna, "circle_angle", PROP_FLOAT, PROP_ANGLE);
	RNA_def_property_float_sdna(prop, NULL, "circle_angle");
	RNA_def_property_range(prop, -FLT_MAX, FLT_MAX);
	RNA_def_property_ui_range(prop, -360.0f, 360.0f, 1.0f, 1);
	RNA_def_property_ui_text(prop, "Angle", "Circle sweep angle");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");
	
		/* Circle segment */
	prop = RNA_def_property(srna, "circle_fill_type", PROP_ENUM, PROP_NONE);
	RNA_def_property_enum_sdna(prop, NULL, "circle_fill_type");
	RNA_def_property_enum_items(prop, rna_enum_circle_fill_items);
	RNA_def_property_ui_text(prop, "Circle Segment", "How much of the circle to fill");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");
	
	prop = RNA_def_property(srna, "circle_sweep_angle", PROP_FLOAT, PROP_ANGLE);
	RNA_def_property_float_sdna(prop, NULL, "circle_angle");
	RNA_def_property_range(prop, -360.0f, 360.0f);
	RNA_def_property_ui_range(prop, -360.0f, 360.0f, 1.0f, 1);
	RNA_def_property_ui_text(prop, "Sweep Angle", "Arc sweep angle in degrees");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");
	
	prop = RNA_def_property(srna, "transform_rot", PROP_FLOAT, PROP_EULER);
	RNA_def_property_float_sdna(prop, NULL, "transform_rot");
	RNA_def_property_ui_text(prop, "Rotation", "Rotation progression per copy");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	prop = RNA_def_property(srna, "transform_scale", PROP_FLOAT, PROP_XYZ);
	RNA_def_property_float_sdna(prop, NULL, "transform_scale");
	RNA_def_property_ui_text(prop, "Scale", "Scale progression per copy");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");
	
		/* Direct offset property for TRANSFORM shape mode */
	prop = RNA_def_property(srna, "offset", PROP_FLOAT, PROP_TRANSLATION);
	RNA_def_property_float_sdna(prop, NULL, "offset");
	RNA_def_property_ui_text(prop, "Offset", "Translation progression per copy");
	RNA_def_property_ui_range(prop, -FLT_MAX, FLT_MAX, 1, RNA_TRANSLATION_PREC_DEFAULT);
	RNA_def_property_update(prop, 0, "rna_Modifier_update");
	
	prop = RNA_def_property(srna, "transform_ref", PROP_ENUM, PROP_NONE);
	RNA_def_property_enum_sdna(prop, NULL, "transform_ref");
	RNA_def_property_enum_items(prop, rna_enum_transform_ref_items);
	RNA_def_property_ui_text(prop, "Transform Reference", "How transformation progression is calculated");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");
	
		/* Randomizer properties */
	prop = RNA_def_property(srna, "random_offset", PROP_FLOAT, PROP_TRANSLATION);
	RNA_def_property_float_sdna(prop, NULL, "random_offset");
	RNA_def_property_ui_text(prop, "Random Offset", "Random offset per copy");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	prop = RNA_def_property(srna, "random_rot", PROP_FLOAT, PROP_EULER);
	RNA_def_property_float_sdna(prop, NULL, "random_rot");
	RNA_def_property_ui_text(prop, "Random Rotation", "Random rotation per copy");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	prop = RNA_def_property(srna, "random_scale_uniform", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "random_scale_uniform");
	RNA_def_property_range(prop, 0.0f, 1.0f);
	RNA_def_property_ui_text(prop, "Random Scale", "Random uniform scale per copy");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	prop = RNA_def_property(srna, "random_scale", PROP_FLOAT, PROP_XYZ);
	RNA_def_property_float_sdna(prop, NULL, "random_scale");
	RNA_def_property_ui_text(prop, "Random Scale Axes", "Random per-axis scale");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	prop = RNA_def_property(srna, "random_flip", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "random_flip", 1);
	RNA_def_property_ui_text(prop, "Flip", "Randomly flip copies");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	prop = RNA_def_property(srna, "random_exclude_first", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "random_exclude_first", 1);
	RNA_def_property_ui_text(prop, "Exclude First", "Don't randomize first copy");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");

	prop = RNA_def_property(srna, "random_exclude_last", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "random_exclude_last", 1);
	RNA_def_property_ui_text(prop, "Exclude Last", "Don't randomize last copy");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");
	...
	/* === EXISTING PROPERTIES CONTINUE HERE === */
...
static void rna_def_modifier_triangulate(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "min_vertices", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "min_vertices");
	RNA_def_property_range(prop, 3, INT_MAX);
	RNA_def_property_ui_range(prop, 3, 10, 1, -1);
	RNA_def_property_ui_text(prop, "Minimum Vertices", "Triangulate only faces with more than this many vertices");
	RNA_def_property_update(prop, 0, "rna_Modifier_update");
}
...
static void rna_def_modifier_noise(BlenderRNA *brna)
{
    ...
    RNA_def_struct_ui_text(srna, "Noise Modifier", "3ds Max style noise deformation");
    RNA_def_struct_sdna(srna, "NoiseModifierData");
    RNA_def_struct_ui_icon(srna, ICON_MOD_WAVE);

    /* Existing properties... */
    prop = RNA_def_property(srna, "strength", PROP_FLOAT, PROP_XYZ);
    RNA_def_property_float_sdna(prop, NULL, "strength");
    RNA_def_property_array(prop, 3);
    RNA_def_property_range(prop, -100.0f, 100.0f);
    RNA_def_property_ui_range(prop, -10.0f, 10.0f, 0.1f, 3);
    RNA_def_property_ui_text(prop, "Strength", "Noise strength per axis");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "scale", PROP_FLOAT, PROP_NONE);
    RNA_def_property_float_sdna(prop, NULL, "scale");
    RNA_def_property_range(prop, 0.0001f, 100.0f);
    RNA_def_property_ui_range(prop, 0.001f, 10.0f, 0.1f, 3);
    RNA_def_property_ui_text(prop, "Scale", "Noise scale");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
    ...
    /* New fractal properties */
    prop = RNA_def_property(srna, "fractal", PROP_BOOLEAN, PROP_NONE);
    RNA_def_property_boolean_sdna(prop, NULL, "fractal", 1);
    RNA_def_property_ui_text(prop, "Fractal", "Enable fractal noise");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
    ...
    RNA_def_property_ui_text(prop, "Roughness", "Fractal roughness");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
    ...
    RNA_def_property_ui_text(prop, "Iterations", "Fractal octaves");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
}

static void rna_def_modifier_capholes(BlenderRNA *brna)
{
    StructRNA *srna;
    PropertyRNA *prop;

	srna = RNA_def_struct(brna, "CapHolesModifier", "Modifier");
    RNA_def_struct_ui_text(srna, "Cap Holes Modifier", "Fill holes in mesh");
    RNA_def_struct_sdna(srna, "CapHolesModifierData");
    RNA_def_struct_ui_icon(srna, ICON_MOD_MASK);

    prop = RNA_def_property(srna, "smooth", PROP_BOOLEAN, PROP_NONE);
    RNA_def_property_boolean_sdna(prop, NULL, "smooth", 1);
    RNA_def_property_ui_text(prop, "Smooth", "Smooth the cap faces");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "triangulate", PROP_BOOLEAN, PROP_NONE);
    RNA_def_property_boolean_sdna(prop, NULL, "triangulate", 1);
    RNA_def_property_ui_text(prop, "Triangulate", "Triangulate the cap");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
}

static void rna_def_modifier_spherify(BlenderRNA *brna)
{
    StructRNA *srna;
    PropertyRNA *prop;

    srna = RNA_def_struct(brna, "SpherifyModifier", "Modifier");
    RNA_def_struct_ui_text(srna, "Spherify Modifier", "Deform mesh into a sphere");
    RNA_def_struct_sdna(srna, "SpherifyModifierData");
    RNA_def_struct_ui_icon(srna, ICON_MOD_CAST);

    prop = RNA_def_property(srna, "percent", PROP_FLOAT, PROP_FACTOR);
    RNA_def_property_float_sdna(prop, NULL, "percent");
    RNA_def_property_range(prop, 0.0f, 1.0f);
    RNA_def_property_ui_range(prop, 0.0f, 1.0f, 0.01f, 3);
    RNA_def_property_ui_text(prop, "Percent", "Amount of spherification");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
    ...
    RNA_def_property_ui_text(prop, "Radius", "Target radius (0 = auto)");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
}

static void rna_def_modifier_push(BlenderRNA *brna)
{
    StructRNA *srna;
    PropertyRNA *prop;

    srna = RNA_def_struct(brna, "PushModifier", "Modifier");
    RNA_def_struct_ui_text(srna, "Push Modifier", "Push vertices along their normals");
    RNA_def_struct_sdna(srna, "PushModifierData");
    RNA_def_struct_ui_icon(srna, ICON_MOD_DISPLACE);

    prop = RNA_def_property(srna, "distance", PROP_FLOAT, PROP_DISTANCE);
    RNA_def_property_float_sdna(prop, NULL, "distance"); 
    RNA_def_property_range(prop, -1000.0f, 1000.0f);
    RNA_def_property_ui_range(prop, -10.0f, 10.0f, 0.01f, 3);
    RNA_def_property_ui_text(prop, "Distance", "Distance to push vertices");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
}

static void rna_def_modifier_twist(BlenderRNA *brna)
{
    StructRNA *srna;
    PropertyRNA *prop;

    srna = RNA_def_struct(brna, "TwistModifier", "Modifier");
    RNA_def_struct_ui_text(srna, "Twist Modifier", "Twist vertices around an axis");
    RNA_def_struct_sdna(srna, "TwistModifierData");
    RNA_def_struct_ui_icon(srna, ICON_MOD_SIMPLEDEFORM);
    ...
    RNA_def_property_range(prop, -FLT_MAX, FLT_MAX);
    RNA_def_property_ui_range(prop, -360.0f, 360.0f, 1.0f, 1);
    RNA_def_property_ui_text(prop, "Angle", "Twist angle in degrees");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "axis", PROP_ENUM, PROP_NONE);
    RNA_def_property_enum_sdna(prop, NULL, "axis");
    /* FIX: Use the globally available 2.79 XYZ axis array */
    RNA_def_property_enum_items(prop, rna_enum_axis_xyz_items); 
    RNA_def_property_ui_text(prop, "Axis", "Axis to twist around");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

}

static void rna_def_modifier_deletemesh(BlenderRNA *brna)
{
    StructRNA *srna;

    /* Registers the modifier structure name with Blender's RNA runtime */
    srna = RNA_def_struct(brna, "DeleteMeshModifier", "Modifier");
    RNA_def_struct_ui_text(srna, "Delete Mesh Modifier", "Completely delete the object mesh topology");
    RNA_def_struct_sdna(srna, "DeleteMeshModifierData");
    
    /* Assigns a red X style icon matching the destructive theme */
    RNA_def_struct_ui_icon(srna, ICON_X); 
}

static const EnumPropertyItem rna_enum_taper_effect_items[] = {
	{0, "X", 0, "X", "Taper only along the first cross-section axis"},
	{1, "Y", 0, "Y", "Taper only along the second cross-section axis"},
	{2, "XY", 0, "XY", "Taper uniformly across both cross-section axes"},
	{0, NULL, 0, NULL, NULL}
};

static void rna_def_modifier_taper(BlenderRNA *brna)
{
    StructRNA *srna;
    PropertyRNA *prop;

    srna = RNA_def_struct(brna, "TaperModifier", "Modifier");
    RNA_def_struct_ui_text(srna, "Taper Modifier", "Scale mesh cross-sections along an axis");
    RNA_def_struct_sdna(srna, "TaperModifierData");
    RNA_def_struct_ui_icon(srna, ICON_MOD_SIMPLEDEFORM);

    prop = RNA_def_property(srna, "amount", PROP_FLOAT, PROP_NONE);
    RNA_def_property_float_sdna(prop, NULL, "amount");
    RNA_def_property_range(prop, -FLT_MAX, FLT_MAX);
    RNA_def_property_ui_range(prop, -10.0f, 10.0f, 0.05f, 2);
    RNA_def_property_ui_text(prop, "Amount", "Taper expansion/pinch amount");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "curve", PROP_FLOAT, PROP_NONE);
    RNA_def_property_float_sdna(prop, NULL, "curve");
    RNA_def_property_range(prop, -FLT_MAX, FLT_MAX);
    RNA_def_property_ui_range(prop, -5.0f, 5.0f, 0.05f, 2);
    RNA_def_property_ui_text(prop, "Curve", "Curvature bowing effect amount");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "axis", PROP_ENUM, PROP_NONE);
    RNA_def_property_enum_sdna(prop, NULL, "axis");
    RNA_def_property_enum_items(prop, rna_enum_axis_xyz_items); 
    RNA_def_property_ui_text(prop, "Primary Axis", "Spine axis along which the taper progresses");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "effect_axis", PROP_ENUM, PROP_NONE);
    RNA_def_property_enum_sdna(prop, NULL, "effect_axis");
    RNA_def_property_enum_items(prop, rna_enum_taper_effect_items); 
    RNA_def_property_ui_text(prop, "Effect", "Cross-section dimension that gets scaled");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    /* FIX: Swapped 'use_symmetry' out for 'taper_symmetry' to bypass the RNA naming collision */
    prop = RNA_def_property(srna, "taper_symmetry", PROP_BOOLEAN, PROP_NONE);
    RNA_def_property_boolean_sdna(prop, NULL, "flag", MOD_TAPER_SYMMETRY);
    RNA_def_property_ui_text(prop, "Symmetry", "Mirror the taper symmetrically across the origin center");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
}

static EnumPropertyItem rna_enum_squeeze_axis_items[] = {
    {0, "X", 0, "X", "Squeeze along X axis"},
    {1, "Y", 0, "Y", "Squeeze along Y axis"},
    {2, "Z", 0, "Z", "Squeeze along Z axis"},
    {0, NULL, 0, NULL, NULL}
};

static void rna_def_modifier_squeeze(BlenderRNA *brna)
{
    StructRNA *srna;
    PropertyRNA *prop;

    srna = RNA_def_struct(brna, "SqueezeModifier", "Modifier");
    RNA_def_struct_ui_text(srna, "Squeeze Modifier", "3ds Max style squeeze deformation");
    RNA_def_struct_sdna(srna, "SqueezeModifierData");
    RNA_def_struct_ui_icon(srna, ICON_MOD_SIMPLEDEFORM);

    /* Radial Squeeze Group */
    prop = RNA_def_property(srna, "amount", PROP_FLOAT, PROP_FACTOR);
    RNA_def_property_float_sdna(prop, NULL, "amount");
    RNA_def_property_range(prop, -FLT_MAX, FLT_MAX);
    RNA_def_property_ui_range(prop, -2.0f, 2.0f, 0.01f, 3);
    RNA_def_property_ui_text(prop, "Amount", "Radial squeeze (positive pinches, negative expands)");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "radial_curve", PROP_FLOAT, PROP_FACTOR);
    RNA_def_property_float_sdna(prop, NULL, "radial_curve");
    RNA_def_property_range(prop, -FLT_MAX, FLT_MAX);
    RNA_def_property_ui_range(prop, -2.0f, 2.0f, 0.01f, 3);
    RNA_def_property_ui_text(prop, "Curve", "Radial curve profile");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    /* Axial Bulge Group */
    prop = RNA_def_property(srna, "axial_amount", PROP_FLOAT, PROP_FACTOR);
    RNA_def_property_float_sdna(prop, NULL, "axial_amount");
    RNA_def_property_range(prop, -FLT_MAX, FLT_MAX);
    RNA_def_property_ui_range(prop, -2.0f, 2.0f, 0.01f, 3);
    RNA_def_property_ui_text(prop, "Amount", "Axial bulge (positive balloons, negative constricts)");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "axial_curve", PROP_FLOAT, PROP_FACTOR);
    RNA_def_property_float_sdna(prop, NULL, "axial_curve");
    RNA_def_property_range(prop, -FLT_MAX, FLT_MAX);
    RNA_def_property_ui_range(prop, -2.0f, 2.0f, 0.01f, 3);
    RNA_def_property_ui_text(prop, "Curve", "Axial curve slope");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    /* Options */
    prop = RNA_def_property(srna, "use_volume_preserve", PROP_BOOLEAN, PROP_NONE);
    RNA_def_property_boolean_sdna(prop, NULL, "flag", MOD_SQUEEZE_VOLUME);
    RNA_def_property_ui_text(prop, "Volume Preserve", "Maintain approximate volume");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "axis", PROP_ENUM, PROP_NONE);
    RNA_def_property_enum_sdna(prop, NULL, "axis");
    RNA_def_property_enum_items(prop, rna_enum_squeeze_axis_items);
    RNA_def_property_ui_text(prop, "Axis", "Primary squeeze axis");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
}

static EnumPropertyItem rna_enum_wavemax_axis_items[] = {
    {0, "X", 0, "X", "Wave along X axis"},
    {1, "Y", 0, "Y", "Wave along Y axis"},
    {2, "Z", 0, "Z", "Wave along Z axis"},
    {0, NULL, 0, NULL, NULL}
};

static const EnumPropertyItem rna_enum_wavemax_mode_items[] = {
    {0, "WAVE", 0, "Wave", "Linear parallel waves traveling across the gizmo space"},
    {1, "RIPPLE", 0, "Ripple", "Concentric radial ripples pushing out from the center"},
    {2, "MIXED", 0, "Mixed Blend", "Interpolate between parallel waves and radial ripples"},
    {0, NULL, 0, NULL, NULL}
};

static void rna_def_modifier_wavemax(BlenderRNA *brna)
{
    StructRNA *srna;
    PropertyRNA *prop;

    srna = RNA_def_struct(brna, "WaveMaxModifier", "Modifier");
    RNA_def_struct_ui_text(srna, "Wave Max Modifier", "3ds Max style parametric Wave, Ripple, and Blended deformer");
    RNA_def_struct_sdna(srna, "WaveMaxModifierData");
    RNA_def_struct_ui_icon(srna, ICON_MOD_WAVE);

    prop = RNA_def_property(srna, "amplitude", PROP_FLOAT, PROP_DISTANCE);
    RNA_def_property_float_sdna(prop, NULL, "amplitude");
    RNA_def_property_array(prop, 2);
    RNA_def_property_range(prop, -FLT_MAX, FLT_MAX);
    RNA_def_property_ui_range(prop, -10.0f, 10.0f, 0.01f, 3);
    RNA_def_property_ui_text(prop, "Amplitude", "Wave amplitude (Amplitude 1, Amplitude 2)");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "wave_length", PROP_FLOAT, PROP_DISTANCE);
    RNA_def_property_float_sdna(prop, NULL, "wave_length");
    RNA_def_property_range(prop, 0.0001f, FLT_MAX);
    RNA_def_property_ui_range(prop, 0.001f, 10.0f, 0.1f, 3);
    RNA_def_property_ui_text(prop, "Wave Length", "Length of each wave");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
    ...
    RNA_def_property_range(prop, -FLT_MAX, FLT_MAX);
    RNA_def_property_ui_range(prop, -10.0f, 10.0f, 0.1f, 3);
    RNA_def_property_ui_text(prop, "Phase", "Wave phase offset");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "decay", PROP_FLOAT, PROP_NONE);
    RNA_def_property_float_sdna(prop, NULL, "decay");
    RNA_def_property_range(prop, 0.0f, 10.0f);
    RNA_def_property_ui_range(prop, 0.0f, 1.0f, 0.01f, 3);
    RNA_def_property_ui_text(prop, "Decay", "Wave exponential decay with distance");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "axis", PROP_ENUM, PROP_NONE);
    RNA_def_property_enum_sdna(prop, NULL, "axis");
    RNA_def_property_enum_items(prop, rna_enum_wavemax_axis_items);
    RNA_def_property_ui_text(prop, "Axis", "Wave travel axis");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    /* 1. Mode Selection Button Registration */
    prop = RNA_def_property(srna, "mode", PROP_ENUM, PROP_NONE);
    RNA_def_property_enum_sdna(prop, NULL, "mode");
    RNA_def_property_enum_items(prop, rna_enum_wavemax_mode_items);
    RNA_def_property_ui_text(prop, "Mode", "Select wave distribution style");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    /* 2. Blending Factor Slider (for Mixed Mode) */
    prop = RNA_def_property(srna, "blend", PROP_FLOAT, PROP_NONE);
    RNA_def_property_float_sdna(prop, NULL, "blend");
    RNA_def_property_range(prop, 0.0f, 1.0f);
    RNA_def_property_ui_range(prop, 0.0f, 1.0f, 0.01f, 3);
    RNA_def_property_ui_text(prop, "Blend Amount", "Balance ratio between Wave (0.0) and Ripple (1.0)");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
}

static EnumPropertyItem rna_enum_melt_axis_items[] = {
    {0, "X", 0, "X", "Melt along X axis"},
    {1, "Y", 0, "Y", "Melt along Y axis"},
    {2, "Z", 0, "Z", "Melt along Z axis"},
    {0, NULL, 0, NULL, NULL}
};

static EnumPropertyItem rna_enum_melt_solidity_items[] = {
    {0, "ICE", 0, "Ice", "Hard, brittle melting (sharp edges)"},
    {1, "GLASS", 0, "Glass", "Smooth, uniform melting"},
    {2, "JELLY", 0, "Jelly", "Soft, wobbly melting (spreads quickly)"},
    {3, "PLASTIC", 0, "Plastic", "Medium stiffness melting"},
    {4, "CUSTOM", 0, "Custom", "Use custom solidity exponent"},
    {0, NULL, 0, NULL, NULL}
};

static void rna_def_modifier_melt(BlenderRNA *brna)
{
    StructRNA *srna;
    PropertyRNA *prop;

    srna = RNA_def_struct(brna, "MeltModifier", "Modifier");
    RNA_def_struct_ui_text(srna, "Melt Modifier", "Melt vertices toward gizmo");
    RNA_def_struct_sdna(srna, "MeltModifierData");
    RNA_def_struct_ui_icon(srna, ICON_MOD_SIMPLEDEFORM);

    prop = RNA_def_property(srna, "gizmo", PROP_POINTER, PROP_NONE);
    RNA_def_property_pointer_sdna(prop, NULL, "gizmo");
    RNA_def_property_ui_text(prop, "Gizmo", "Empty object controlling melt origin");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
    ...
    RNA_def_property_range(prop, 0.0f, 1.0f);
    RNA_def_property_ui_range(prop, 0.0f, 1.0f, 0.01f, 3);
    RNA_def_property_ui_text(prop, "Amount", "Melt amount");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
    ...
    RNA_def_property_ui_text(prop, "Spread", "Base spreading");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "solidity_type", PROP_ENUM, PROP_NONE);
    RNA_def_property_enum_sdna(prop, NULL, "solidity_type");
    RNA_def_property_enum_items(prop, rna_enum_melt_solidity_items);
    RNA_def_property_ui_text(prop, "Solidity", "Material solidity preset");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "solidity_val", PROP_FLOAT, PROP_NONE);
    RNA_def_property_float_sdna(prop, NULL, "solidity_val");
    RNA_def_property_range(prop, 0.01f, 10.0f);
    RNA_def_property_ui_range(prop, 0.1f, 5.0f, 0.01f, 3);
    RNA_def_property_ui_text(prop, "Solidity Exponent", "Custom solidity exponent");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");

    prop = RNA_def_property(srna, "axis", PROP_ENUM, PROP_NONE);
    RNA_def_property_enum_sdna(prop, NULL, "axis");
    RNA_def_property_enum_items(prop, rna_enum_melt_axis_items);
    RNA_def_property_ui_text(prop, "Axis", "Gravity direction");
    RNA_def_property_update(prop, 0, "rna_Modifier_update");
}
...
void RNA_def_modifier(BlenderRNA *brna)
{
	...
	rna_def_modifier_capholes(brna);
	rna_def_modifier_spherify(brna);
	rna_def_modifier_push(brna);
	rna_def_modifier_twist(brna);
	rna_def_modifier_deletemesh(brna); 
	rna_def_modifier_taper(brna);
