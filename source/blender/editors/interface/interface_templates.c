...
{
	uiBut *but = arg1;
	unsigned int cur = (unsigned int)(uintptr_t)arg2;
	wmWindow *win = CTX_wm_window(C);
	Scene *scene = CTX_data_scene(C);
	View3D *v3d = CTX_wm_view3d(C);
	int shift = win->eventstate->shift;

	/* Dynamic layer button - no RNA pointer */
	if (!but->rnapoin.data || !but->rnaprop) {
		...
		if (shift) {
			...
			for (SceneLayer *tmp = scene->layers.first; tmp; tmp = tmp->next) {
				if (tmp->index == (int)cur) {
					tmp->flag |= SCE_LAYER_FLAG_VISIBLE;
				}
				else {
					tmp->flag &= ~SCE_LAYER_FLAG_VISIBLE;
				}
	...
	/* Old fixed 20-layer system */
	if (!shift) {
		int i, tot;
		tot = RNA_property_array_length(&but->rnapoin, but->rnaprop);

		RNA_property_boolean_set_index(&but->rnapoin, but->rnaprop, (int)cur, true);
		for (i = 0; i < tot; ++i) {
			if ((unsigned int)i != cur)
				RNA_property_boolean_set_index(&but->rnapoin, but->rnaprop, i, false);
		}
	}
}

static uiBlock *ui_layer_context_menu(bContext *C, ARegion *ar, void *UNUSED(arg))
{
	uiBlock *block;
	uiLayout *layout;
	uiStyle *style = UI_style_get();

	block = UI_block_begin(C, ar, "layer_context_menu", UI_EMBOSS);
	layout = UI_block_layout(block, UI_LAYOUT_VERTICAL, UI_LAYOUT_MENU, 0, 0, 120, 20, 0, style);

	uiItemO(layout, "Add Layer", ICON_ZOOMIN, "VIEW3D_OT_layer_add");
	uiItemO(layout, "Remove Layer", ICON_X, "VIEW3D_OT_layer_remove");
...
static void layer_add_with_warp_cb(bContext *C, void *scene_v, void *UNUSED(arg))
{
	Scene *scene = (Scene *)scene_v;
	SceneLayer *sl;
	int layer_count = 0;
	...
	/* Cache cursor positions safely before modifying application state context */
	wmWindow *win = CTX_wm_window(C);
	int mouse_x = win->eventstate->x;
	int mouse_y = win->eventstate->y;

	/* Call operator */
	WM_operator_name_call(C, "VIEW3D_OT_layer_add", WM_OP_EXEC_DEFAULT, NULL);

	WM_event_add_notifier(C, NC_SCENE | ND_LAYER, scene);

	int old_pos = layer_count;     /* Index where '+' button was */

	int dx = 0, dy = 0;

	/*
	 * Exact Base Unit Sizes:
	 * Represents the theoretical width allocations for single icon buttons.
	 */
	float standard_unit = UI_UNIT_X / 2.0f;

	if (old_pos % 10 == 4) {
		/*
		 * Move from Top Row (Col 4) down to Bottom Row (Col 0)
		 *
		 * DETAILED BREAKDOWN:
		 * 1. Distance from Col 0 center to Col 4 center is exactly 4 layout columns.
		 * 2. When aligned (true), Blender compresses boundaries by ~1.5 to 2 pixels
		 *    per button block interface boundary depending on theme scale.
		 *
		 * FIX: Multiply by exactly 3.82f. This completely negates the layout compression
		 * loss and drops the mouse dead-center on the target button instead of its left wall.
		 */
		dx = -(int)(3.82f * standard_unit);
		dy = -(UI_UNIT_Y / 2); /* Negative Y moves down */
	}
	else if (old_pos % 10 == 9) {
		/*
		 * Move from Bottom Row (Col 4) of Group N
		 * to Top Row (Col 0) of Group N+1.
		 *
		 * Visually, Column 0 of Group N+1 sits right next to Column 4 of Group N.
		 * Just step right over the block boundary partition and jump UP.
		 */
		dx = (int)(1.44f * standard_unit);
		dy = UI_UNIT_Y / 2;    /* Positive Y moves up */
	}
	else {
		/* Default behavior: Step right to the next item within the same row container */
		dx = (int)standard_unit;
	}

	WM_cursor_warp(win, mouse_x + dx, mouse_y + dy);
        ...
        PointerRNA *used_ptr, const char *used_propname, int active_layer)
{
	...
	if (ptr->id.data && GS(((ID *)ptr->id.data)->name) == ID_SCE) {
		...
		/* Calculate how many groups of 10 we need */
		...
		for (int group = 0; group < group_count; group++) {
			...
			for (int row = 0; row < 2; row++) {
				...
				for (int col = 0; col < 5; col++) {
					...
					sl = BKE_scene_layer_find_index(scene, layer_index);
					...
					if (sl) {
						...
						/* else icon stays 0 (empty) */

						but = uiDefIconBut(
							block, UI_BTYPE_TOGGLE, 0, icon,
							0, 0, UI_UNIT_X / 2, UI_UNIT_Y / 2,
							NULL, 0, 0, 0, 0,
							sl->name);

						/* Set pressed state based on visibility */
						if (sl->flag & SCE_LAYER_FLAG_VISIBLE) {
							but->flag |= UI_SELECT;
						}

						UI_but_func_set(but, handle_layer_buttons, but, POINTER_FROM_INT(sl->index));
					}
					else if (layer_index == layer_count) {
						/* "+" button with cursor warp */
						uiBut *add_but;
						add_but = uiDefIconBut(
							block, UI_BTYPE_BUT, 0, ICON_ZOOMIN,
							0, 0, UI_UNIT_X / 2, UI_UNIT_Y / 2,
							NULL, 0, 0, 0, 0,
							TIP_("Add a new layer"));

						/* Call operator first, then warp cursor */
						UI_but_func_set(add_but, layer_add_with_warp_cb, scene, NULL);
					}
				}
			}
		}
