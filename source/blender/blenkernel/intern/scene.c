...
void BKE_scene_init(Scene *sce)
{
	...
	//custom vibecoded addition for now
	/* Initialize dynamic layer system */
	sce->layers.first = NULL;
	sce->layers.last = NULL;
	sce->active_layer = 0;
	
	/* Create default 20 layers (matching original Blender behavior) */
	for (int i = 0; i < 20; i++) {
		SceneLayer *sl = BKE_scene_layer_add(sce, "");
		BLI_snprintf(sl->name, sizeof(sl->name), "Layer %d", i + 1);
		sl->flag = SCE_LAYER_FLAG_VISIBLE;  /* All visible by default */
		sl->color[0] = 0.6f;
		sl->color[1] = 0.6f;
		sl->color[2] = 0.6f;
	}
...
//vibecoded uhh i forgot what it means
SceneLayer *BKE_scene_layer_add(Scene *scene, const char *name)
{
    SceneLayer *sl = MEM_callocN(sizeof(SceneLayer), "SceneLayer");
    ...
    sl->index = BLI_listbase_count(&scene->layers);
    sl->flag = SCE_LAYER_FLAG_VISIBLE;  /* Default: visible */
    
    BLI_addtail(&scene->layers, sl);
    return sl;
}

/* Find a SceneLayer by its index */
SceneLayer *BKE_scene_layer_find_index(const Scene *scene, const int index)
{
    SceneLayer *sl;
...
/* Find the active SceneLayer */
SceneLayer *BKE_scene_layer_find_active(const Scene *scene)
{
    if (scene == NULL) {
        return NULL;
    }
    
    return BKE_scene_layer_find_index(scene, scene->active_layer);
}

/* there was a debugger there. not needed anymore */
bool BKE_scene_layer_is_visible(const Scene *scene, const int layer_index)
{
    SceneLayer *sl = BKE_scene_layer_find_index(scene, layer_index);
    
    if (sl == NULL) return false;
    return (sl->flag & SCE_LAYER_FLAG_VISIBLE) != 0;
}

/* Set a layer's visibility */
void BKE_scene_layer_set_visible(Scene *scene, const int layer_index, const bool visible)
{
    SceneLayer *sl;
...
void BKE_scene_layer_remove(Scene *scene, SceneLayer *sl)
{
    if (scene == NULL || sl == NULL) {
        return;
    }
    ...
    if (scene->active_layer == sl->index) {
        SceneLayer *new_active = scene->layers.first;
        if (new_active) {
            scene->active_layer = new_active->index;
        }
        else {
...
void BKE_scene_set_background(Main *bmain, Scene *scene)
{
	...
	/* Ensure dynamic layers exist (for old files or files without them) */
	if (BLI_listbase_is_empty(&scene->layers)) {
		for (int i = 0; i < 20; i++) {
