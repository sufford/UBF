...
#include "BKE_main.h" /* another own include lmao */
#include "BKE_object.h"
#include "BKE_object_deform.h"
...
//HOOKER
//i think every options ill add should be listed bottom and not anywhere in middle or somewhere else. it will make things easier

static int view3d_layer_add_exec(bContext *C, wmOperator *UNUSED(op))
{
	Scene *scene = CTX_data_scene(C);
	...
	if (fp) {
		fprintf(fp, "Added layer: %s (total: %d)\n", name, BLI_listbase_count(&scene->layers));
		fclose(fp);
	}
...
static int view3d_layer_remove_exec(bContext *C, wmOperator *UNUSED(op))
{
	...
	if (sl) {
		...
		for (ob = bmain->object.first; ob; ob = ob->id.next) {
			ObjectLayerLink *oll, *oll_next;
			for (oll = ob->layer_links.first; oll; oll = oll_next) {
				oll_next = oll->next;
				if (oll->layer == sl) {
					BLI_remlink(&ob->layer_links, oll);
					MEM_freeN(oll);
				}
			}
		}
		...
		if (scene->active_layer >= BLI_listbase_count(&scene->layers)) {
			scene->active_layer = MAX2(0, BLI_listbase_count(&scene->layers) - 1);
		}
...
//watch it will cause compilation errors
void VIEW3D_OT_layer_add(wmOperatorType *ot)
{
	ot->name = "Add Layer";
	ot->description = "Add a new layer to the scene";
	ot->idname = "VIEW3D_OT_layer_add";
	ot->exec = view3d_layer_add_exec;
	ot->poll = ED_operator_view3d_active;
	ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO;
}


/* Operator registration SECOND */
void VIEW3D_OT_layer_remove(wmOperatorType *ot)
{
	ot->name = "Remove Layer";
	ot->description = "Remove the active layer";
	ot->idname = "VIEW3D_OT_layer_remove";
	ot->exec = view3d_layer_remove_exec;
	ot->poll = ED_operator_view3d_active;
	ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO;
}
