...
#include "BKE_sca.h"
#include "BKE_scene.h"  /* For BKE_scene_layer_find_index - might already be there */
...
/* For UI functions */
#include "UI_interface.h"
#include "UI_interface_icons.h"  /* For ICON_INFO, ICON_NONE */

/* For popup menu functions */
#include "UI_resources.h"  /* UI_pupmenu_begin, etc. */
...
bool ED_object_editmode_enter(bContext *C, int flag)
{
	...
	if ((flag & EM_IGNORE_LAYER) == 0) {
		...
		/* Check if base is visible using dynamic layer support */
		bool is_visible = false;

		/* Old bitmask check */
		if (v3d && (base->lay & v3d->lay)) {
			is_visible = true;
		}
		else if (!v3d && (base->lay & scene->lay)) {
			is_visible = true;
		}

		/* Dynamic layer check */
		if (!is_visible && base->object->layer_links.first) {
	...
	if (ob->type == OB_MESH) {
		...
		/* Force update for dynamic layer objects */
		if (ob->layer_links.first) {
...
//before last function
/************************ Move to Layer (Dynamic) ***********************/

/* Helper: Remove object from all dynamic layers */
static void object_dynamic_layer_clear(Object *ob)
{
	ObjectLayerLink *link, *link_next;

	for (link = ob->layer_links.first; link; link = link_next) {
		link_next = link->next;
		BLI_freelinkN(&ob->layer_links, link);
	}
}

/* Helper: Add object to a dynamic layer */
static void object_dynamic_layer_add(Object *ob, SceneLayer *sl)
{
	ObjectLayerLink *link;
...
/* Helper: Update bitmask for compatibility */
static void object_dynamic_layer_update_bitmask(Object *ob)
{
	ObjectLayerLink *link;
...
/* Move selected objects to a dynamic layer */
static int object_move_to_layer_set_exec(bContext *C, wmOperator *op)
{
	Scene *scene = CTX_data_scene(C);
	int layer_index = RNA_int_get(op->ptr, "layer_index");
	SceneLayer *target_layer = NULL;
	SceneLayer *sl;

	/* Find target layer */
	for (sl = scene->layers.first; sl; sl = sl->next) {
		if (sl->index == layer_index) {
			target_layer = sl;
			break;
		}
	}
	...
	if (!target_layer) {
		...
		target_layer->color[1] = 0.6f;
		target_layer->color[2] = 0.6f;
		BLI_addtail(&scene->layers, target_layer);
	}
	...
	CTX_DATA_BEGIN(C, Base *, base, selected_bases)
	{
		...
		/* Clear all dynamic layer links */
		object_dynamic_layer_clear(ob);

		/* Add to target layer */
		object_dynamic_layer_add(ob, target_layer);

		/* Update bitmask for compatibility */
		object_dynamic_layer_update_bitmask(ob);
...
/* Block menu for grid layout - matches old 20-layer visual style */
static uiBlock *object_move_to_layer_set_menu(bContext *C, ARegion *ar, void *UNUSED(arg))
{
	Scene *scene = CTX_data_scene(C);
	SceneLayer *sl;
	uiBlock *block;
	...
	/* Calculate how many groups of 10 (2 rows x 5 cols) */
	group_count = ((layer_count + 1) / 10) + (((layer_count + 1) % 10) != 0);

	block = UI_block_begin(C, ar, "move_to_layer_set_menu", UI_EMBOSS);

	/* Use UI_LAYOUT_MENU type for proper popup menu styling */
	layout = UI_block_layout(block, UI_LAYOUT_VERTICAL, UI_LAYOUT_MENU, 0, 0, UI_UNIT_X * 8, UI_UNIT_Y * (2 * group_count + 2), 0, UI_style_get());

	/* Title */
	uiItemL(layout, "Move to Layer:", ICON_NONE);
	uiItemS(layout);

	/* Create grid of layer buttons - 2 rows x 5 columns per group */
	for (int group = 0; group < group_count; group++) {
		uiLayout *group_layout = uiLayoutColumn(layout, false);

		for (int row = 0; row < 2; row++) {
			uiLayout *row_layout = uiLayoutRow(group_layout, true);

			for (int col = 0; col < 5; col++) {
				int layer_index = group * 10 + row * 5 + col;

				/* Find the layer */
				sl = NULL;
				for (SceneLayer *tmp = scene->layers.first; tmp; tmp = tmp->next) {
					if (tmp->index == layer_index) {
						sl = tmp;
						break;
					}
				}
				...
				if (sl) {
					/* Button with layer name - width to fit 5 across */
					PointerRNA opptr;
					uiItemFullO_ptr(row_layout, WM_operatortype_find("OBJECT_OT_move_to_layer_set", false),
						sl->name, 0, NULL, WM_OP_EXEC_DEFAULT, 0, &opptr);
					RNA_int_set(&opptr, "layer_index", sl->index);
				}
			}
		}
	...
	UI_block_bounds_set_popup(block, 6, 0, 0);
	UI_block_direction_set(block, UI_DIR_DOWN);

	return block;
}

static int object_move_to_layer_set_invoke(bContext *C, wmOperator *op, const wmEvent *UNUSED(event))
{
	/* Use block menu for grid layout */
	UI_popup_block_invoke(C, object_move_to_layer_set_menu, NULL);

	return OPERATOR_INTERFACE;
}

void OBJECT_OT_move_to_layer_set(wmOperatorType *ot)
{
	/* identifiers */
	ot->name = "Move to Layer (Dynamic)";
	ot->description = "Move selected objects to a dynamic layer";
	ot->idname = "OBJECT_OT_move_to_layer_set";

	/* api callbacks */
	ot->invoke = object_move_to_layer_set_invoke;
	ot->exec = object_move_to_layer_set_exec;
	ot->poll = ED_operator_object_active_editable;
	...
	/* properties */
	RNA_def_int(ot->srna, "layer_index", 0, 0, INT_MAX, "Layer Index", "Index of the dynamic layer to move to", 0, INT_MAX);
}

/* Page switcher for dynamic layers - stub for now */
static int object_layer_page_exec(bContext *C, wmOperator *op)
{
	/* TODO: Implement page switching later */
	Scene *scene = CTX_data_scene(C);
	int direction = RNA_int_get(op->ptr, "direction");

	/* Just cycle active_layer_page for now */
	if (direction > 0) {
		scene->active_layer_page++;
	}
	else {
		scene->active_layer_page--;
	}
	if (scene->active_layer_page < 0) scene->active_layer_page = 0;
...
void OBJECT_OT_layer_page(wmOperatorType *ot)
{
	/* identifiers */
	ot->name = "Layer Page";
	ot->description = "Switch to different layer page";
	ot->idname = "OBJECT_OT_layer_page";

	/* api callbacks */
	ot->exec = object_layer_page_exec;
	ot->poll = ED_operator_object_active_editable;
	...
	/* properties */
	RNA_def_int(ot->srna, "direction", 1, -1, 1, "Direction", "Page switch direction (1=next, -1=prev)", -1, 1);
}
