...
/* Helper to check if base is selectable using dynamic layers */
static bool base_selectable_dynamic(View3D *v3d, Base *base, Scene *scene)
{
	/* Check dynamic layer visibility */
	if (!view3d_base_visible(scene, v3d, base)) {
		return false;
	}
	
	/* Check object restriction flags */
	if (base->object->restrictflag & (OB_RESTRICT_SELECT | OB_RESTRICT_VIEW)) {
		return false;
	}
...
static void object_deselect_all_visible(Scene *scene, View3D *v3d)
{
	...
	for (base = scene->base.first; base; base = base->next) {
		if (base_selectable_dynamic(v3d, base, scene)) {
        ...
        const bool extend, const bool select)
{
	...
	for (base = vc->scene->base.first; base; base = base->next) {
		if (base_selectable_dynamic(vc->v3d, base, vc->scene)) { /* use this to avoid un-needed lasso lookups */
        ...
        bool extend, bool deselect, bool toggle, bool obcenter, bool enumerate, bool object)
{
	...
	if (basact) {
		...
		else if (base_selectable_dynamic(v3d, basact, scene)) {
...
static int do_object_pose_box_select(bContext *C, ViewContext *vc, rcti *rect, bool select, bool extend)
{
	...
	if (hits > 0) { /* no need to loop if there's no hit */
		...
		for (base = vc->scene->base.first; base && hits; base = base->next) {
			if (base_selectable_dynamic(vc->v3d, base, vc->scene)) {
...
static bool object_circle_select(ViewContext *vc, const bool select, const int mval[2], float rad)
{
	...
	Base *base;
	for (base = FIRSTBASE; base; base = base->next) {
		if (base_selectable_dynamic(vc->v3d, base, vc->scene) && ((base->flag & SELECT) != select_flag)) {
			float screen_co[2];
			if (ED_view3d_project_float_global(
